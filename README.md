...
- **Notifications in-app** : `<NotificationBell>` (sidebar agence + en-tête portail), compteur non-lues. Émissions : soumission / validation / renvoi / approbation / refus + `post_awaiting_client` aux contacts + commentaires (trigger `post_comments_notify`, migr 0038, `post_comments.kind`).
- **Auto-publication** (migr 0039) : `app_settings.account.auto_publish` → job `auto-publish` (pg_cron, 10 min) passe les posts planifiés échus en publié. Toggle dans Paramètres › Compte. Bouton « Publié » rapide sur l'accueil.
- **Validation par lien** (migr 0040) : `post_approval_tokens` (1/post, régénéré à `client_review`). Page publique `/valider/:token` (approuver / demander une modif, sans connexion). RPC `post_by_approval_token` / `approve_via_token` / `reject_via_token`. Encart « Lien de validation directe » dans le panneau post.
- **E-mails** (migr 0041 + Edge Function `dispatch-emails`) : job pg_cron (1 min) → `dispatch-emails` (Resend) envoie un e-mail pour chaque notification (`notifications.email_sent_at`). `admin-users` envoie aussi les liens d'invitation. **Config Supabase requise** : `supabase functions deploy dispatch-emails --no-verify-jwt` + `supabase secrets set RESEND_API_KEY=... EMAIL_FROM='Cadence <no-reply@dom>' APP_URL=https://app.dom` (+ `RESEND_API_KEY`/`EMAIL_FROM` sur `admin-users`). Sans clé : no-op, notifications in-app seules.
- **Multi-tenant** (migr 0042 + 0043) : `organizations` + `organization_id` sur toutes les tables métier, rempli par des triggers `tenant_fill_*` (dérivé du parent), isolé par des policies RLS **`RESTRICTIVE` `org_isolation`** (`organization_id = auth_org()`). Réglages **par organisation** via `org_settings` (`account`, `workflow`, `onboarding_template` ; `alert_thresholds` reste global). `platform_admins` = super-admin plateforme. **Inscription sur invitation** : `org_invitations` + RPC `accept_org_invitation` (crée l'orga + le 1er Directeur), page publique `/rejoindre/:token`, action Edge `admin-users` → `invite_org` (platform admin). Voir `docs/stories/post-v1.multi-tenant.md`. **Config Supabase requise** : Redirect URLs `<APP_URL>/rejoindre/*` ; `APP_URL` sur `admin-users`.
- **Didacticiel** (front seul, `src/onboarding/`) : carrousel de présentation en 6 écrans à la première connexion (dont un dédié à **l'espace client**, avec un mock non interactif du portail réutilisé dans l'Aide), puis **visite guidée** en surbrillance de la barre latérale (surimpression maison, cibles `data-tour`). État « déjà vu » en `localStorage` (par appareil). Relançable via **Aide → « Revoir la visite guidée »**. Rôles internes uniquement, desktop pour la visite. Voir `docs/stories/post-v1.onboarding-tour.md`.
- **Upload des logos / photos** (migr 0047) : bucket public `brand-assets` (5 Mo, images) + policies (écriture = authentifié, update/delete = propriétaire). `<ImageUploadField>` remplace les champs « URL » pour le logo client (`ClientForm`), le logo d'agence (`AccountSettingsPage`, `OnboardingWizard`) et la photo de profil (`MyAccountPage`). Les colonnes `*_url` stockent toujours une URL (celle de l'objet uploadé) — pas de changement de schéma. `src/services/brandAssets.ts`. Voir `docs/stories/post-v1.brand-asset-uploads.md`.
- **Jeu de démo par organisation** (migr 0048) : `clients.is_demo` + `seed_org_demo(org, author)` (client « Studio Lumen (démo) » + 3 rubriques + 1 campagne + 7 posts tous statuts), appelé par `accept_org_invitation`. `delete_org_demo()` (Directeur) supprime le tout. `ClientsPage` : bandeau + bouton « Supprimer la démo », puce « démo ». Voir `docs/stories/post-v1.org-demo.md`.
- **Outils d'administration plateforme** (migr 0049) : `platform_list_organizations()` expose désormais le Directeur (nom/e-mail), le nombre de posts et la dernière activité de chaque agence ; `platform_revoke_invitation(token)` et `platform_reset_organization(org)` (supprime le contenu, détache et réinitialise les membres — comptes conservés, ré-invitables). `/app/plateforme` : tuiles de synthèse, bouton Révoquer par invitation, bouton Réinitialiser par agence. Voir `docs/stories/post-v1.platform-admin-tools.md`.
